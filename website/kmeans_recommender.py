...
def prepare_data(books, borrow, search, favorites):
    ...
    content_df = books[['book_id', 'title', 'authors', 'categories', 'copyright']].copy()
...
def get_content_based_recommendations(book_ids, content_df, book_id_to_info, top_n, n_clusters):
    all_recommendations = {}
    ...
    clusters = kmeans.fit_predict(content_matrix_reduced)
    
    content_df['Cluster'] = clusters
    
    # Compute clustering evaluation metrics
    silhouette_avg = silhouette_score(content_matrix_reduced, clusters)
    calinski_harabasz_avg = calinski_harabasz_score(content_matrix_reduced, clusters)
    davies_bouldin_avg = davies_bouldin_score(content_matrix_reduced, clusters)
...
def get_collaborative_filtering_recommendations(user_id, top_n, ratings, book_id_to_info):
    ...
    # Calculate metrics
    true_ratings = user_books[user_books > 0].values
    predicted_ratings = unrated_books.loc[unrated_books.index.isin(user_books.index)].values[:len(true_ratings)]
    
    mse = mean_squared_error(true_ratings, predicted_ratings)
    rmse = np.sqrt(mse)
    mae = mean_absolute_error(true_ratings, predicted_ratings)
    
    # Precision and Recall (Note: Precision and Recall are typically used for binary relevance, 
    # so in this context, it's a simplification to treat predicted ratings above a threshold as positive)
    threshold = 3.0  # Example threshold for a "good" rating
    true_positives = (true_ratings >= threshold).astype(int)
    predicted_positives = (predicted_ratings >= threshold).astype(int)
    ...
    # print(f"Collaborative Filtering Precision: {precision:.4f}")
    # print(f"Collaborative Filtering Recall: {recall:.4f}")
